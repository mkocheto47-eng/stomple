    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(30,28,20,.55)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 40 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 480, maxHeight: '88dvh', overflowY: 'auto', padding: '20px 20px calc(20px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: 12, animation: 'popIn .3s ease' }}>
        ...
        <div style={row}>
          ...
          <p style={p}>Если рядом с затопленным шариком лежат шарики того же цвета, вы <b>обязаны</b> продолжить и утопить их тоже — в любую сторону, хоть с поворотами, пока рядом не останется ни одного. Направление выбираете сами: тапните на дальний шарик, чтобы пройти путь сразу, или идите по одному.</p>
        </div>
        ...
        {h('Тактика')}
        <p style={p}>Берегите свои шарики — они и ваши прыжки, и ваши очки. Белые дороже всего: каждый уцелевший — 3 очка победителю, так что топите их, если побеждать собирается не вы. Прежде чем наступить на шарик, посмотрите, куда утянет цепочка — отказаться от неё нельзя, а вынести вас может в пустоту.</p>
