  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(30,28,20,.55)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 40 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 480, maxHeight: '88dvh', overflowY: 'auto', padding: '20px 20px calc(20px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: 12, animation: 'popIn .3s ease' }}>
        ...
        {h('Выбывание и очки')}
        <p style={p}>Если в свой ход игроку некуда ступить и не на что прыгнуть — он выбывает из раунда. Последний оставшийся получает <b>3 очка</b>, плюс по <b>1</b> за каждый белый и каждый шарик своего цвета, уцелевший на поле.</p>
        ...
        {h('Тактика')}
        <p style={p}>Берегите свои шарики — они и ваши прыжки, и ваши очки. Топите чужие, чтобы лишить соперников прыжков. Не загоняйте себя в угол: стомпер в пустоте без своих шариков — это выбывание.</p>
