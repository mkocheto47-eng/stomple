      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {view.players.map((q, i) => {
          ...
          return <div key={q.id + (on ? turnKey : '')} style={{ outline: speaking ? `3px solid ${COLORS[q.color].hex}` : '3px solid transparent', outlineOffset: 1, ['--c' as any]: COLORS[q.color].hex + '99', flex: 1, background: on ? '#fff' : '#fbf9f3', border: `2px solid ${on ? COLORS[q.color].hex : '#eee8d9'}`, borderRadius: 14, padding: '6px 4px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, opacity: q.alive ? 1 : .38, boxShadow: on ? '0 4px 12px rgba(60,50,20,.15)' : 'none', minWidth: 0, transform: on ? 'translateY(-3px)' : 'none', overflow: 'hidden', transition: 'transform .3s, border-color .3s, background .3s, outline-color .15s', animation: on ? 'chipPulse .9s ease-out 1' : 'none', position: 'relative' }}>
            ...
            {react && <div key={react.key} style={{ position: 'absolute', inset: 0, borderRadius: 12, background: 'rgba(255,255,255,.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 6, pointerEvents: 'none', animation: 'reactIn 2.2s ease forwards' }}>
              <img src={`/reactions/${react.id}.webp`} alt="" style={{ width: '70%', maxWidth: 56, aspectRatio: '1', animation: 'reactBounce 2.2s ease forwards' }} />
            </div>}
